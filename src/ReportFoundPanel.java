...
public class ReportFoundPanel extends JPanel {
    ...
    public ReportFoundPanel(int userId) {
        ...
        JLabel title = UIHelper.title("Report Found Asset");
        title.setBounds(80, 40, 500, 45);
        add(title);

        JLabel sub = UIHelper.subtitle("Submit found item details so RecoverAI can identify the owner faster.");
        sub.setBounds(80, 90, 800, 30);
        add(sub);
        ...
        locationField = addInput(card, "Found Location", 40, 200);
        dateField = addInput(card, "Found Date YYYY-MM-DD", 370, 200);
        ...
        JButton submit = UIHelper.neonButton("Submit Found Report");
        submit.setBounds(230, 470, 220, 38);
        card.add(submit);

        browse.addActionListener(e -> chooseImage());
        submit.addActionListener(e -> saveFoundItem());
    }
    ...
    void saveFoundItem() {
        try {
            ...
            String ocrText = imageField.getText().isEmpty()
                    ? ""
            ...
            JOptionPane.showMessageDialog(this, "Found report submitted with OCR intelligence.");

        } catch (Exception e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error saving found report.");
        }
    }
}